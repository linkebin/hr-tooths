...
public class PotentialCustController {
    ...
    public Result add(String json) {
		...
		potentialCust.setClinic(secUser.getOrgId());
    ...
    public Result delete(@RequestParam String id) {
        potentialCustService.deleteById(id);
        return ResultGenerator.genSuccessResult();
    }
    ...
    @GetMapping("/find")
    public Result findPotentialCust(String source, String starTime, String endTime, String customer,int size,int page) {
    	try {
			SecUser secUser = Security.getUser();
			List<Map<String,Object>> potentialCustList = potentialCustService.findPotentialCust(source,secUser.getOrgId(),starTime,endTime,customer,size,(page - 1)*size);
			int count = potentialCustService.findPotentialCustCount(source,secUser.getOrgId(),starTime,endTime,customer);
	...
	public void excelCust(HttpServletResponse response) throws Exception {
		...
		List<Map<String,Object>> list = potentialCustService.findPotentialCustAll("",secUser.getOrgId(),"","","");
