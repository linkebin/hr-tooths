...
public class LoginController {
   ...
   @Resource
    ...
    public Result authorize(HttpServletRequest request, String username, String password){
        ...
        daoService.getDivMap("update sec_user set STANDBY2=0 where ID_='"+secUser.getId()+"'");
...
//        resource.set((String)  SecurityUtils.getSubject().getSession().getId(),Security.getUserId());
    ...
    public  Result checkPwd(String username,String password){
        try{
            ...
            if (user.getUserPass().equals(secUser.getUserPass())){
   ...
   @PostMapping("/getOrgName")
    public  Result getOrgName(String account){
        try{
            SecOrg secOrg = daoService.getObject("SELECT * from sec_org where ID_ = (SELECT ORG_ID FROM `sec_user` where ACCOUNT ='" + account + "')", SecOrg.class);
            if("门诊".equals(secOrg.getType())){
                return ResultGenerator.genSuccessResult(secOrg.getOrgName());
            }else {
                return ResultGenerator.genSuccessResult("总部账号，请切换至管理端");
            }
